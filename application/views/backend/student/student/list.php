...
<table id="basic-datatable" class="table table-striped dt-responsive nowrap" width="100%">
    ...
    ?>
        <tr>
            ...
            <td>
                <button type="button" class="btn btn-icon btn-secondary btn-sm btn-dark" style="margin-right:5px;" onclick="largeModal('<?php echo site_url('modal/popup/student/profile/'.$student['id'])?>', '<?php echo $this->db->get_where('schools', array('id' => $school_id))->row('name'); ?>')" data-toggle="tooltip" data-placement="top" title="" data-original-title="<?php echo get_phrase('student_profile'); ?>"> <i class="dripicons-checklist"></i></button>

                <a href="<?php echo route('student/edit/'.$student['id']); ?>" class="btn btn-icon btn-secondary btn-sm btn-dark" style="margin-right:5px;" data-toggle="tooltip" data-placement="top" title="" data-original-title="<?php echo get_phrase('update_student_information'); ?>"> <i class="mdi mdi-wrench"></i></a>

                <button id="uname" type="button" class="btn btn-icon btn-secondary btn-sm tooltip_togle" style="margin-right:5px;" onclick="confirmModal('<?php echo route('student/delete/'.$student['id'].'/'.$student['user_id']); ?>', showAllStudents)" data-toggle="tooltip" data-placement="top" title="" data-original-title="<?php echo get_phrase('delete_student'); ?>"> <i class="mdi mdi-window-close"></i></button>
            </td>
        </tr>
    <?php } ?>
    </tbody>
