...
class User_model extends CI_Model {
	...
	public function single_student_create(){
		...
		// check email duplication
		$duplication_status = $this->check_duplication('on_create');
		if($duplication_status){
			...
			$student_data['joining'] = strtotime(html_escape($this->input->post('joining')));
		...
		if (($handle = fopen('uploads/csv_file/student.generate.csv', 'r')) !== FALSE) { // Check the resource is valid
			...
			while (($all_data = fgetcsv($handle, 1000, ",")) !== FALSE) { // Check opening the file is OK!
				if($count > 0){
					...
					$user_data['birthday'] = html_escape($all_data[1]);
					$user_data['blood_group'] = html_escape($all_data[2]);
					$user_data['phone'] = html_escape($all_data[3]);
					$user_data['address'] = html_escape($all_data[4]);
					$user_data['gender'] = html_escape($all_data[5]);
					$user_data['joining'] = html_escape($all_data[6]);
					$user_data['role'] = $role;
					...
					if($duplication_status){
						...
						$student_data['joining'] = html_escape($all_data[6]);
						$this->db->insert('students', $student_data);
						$student_id = $this->db->insert_id();
